...
int ourRTSPClient::createVideoFile(){
	...
	if (strcmp(filename_suffix, "mp4") == 0){
		...
		Out0 = QuickTimeFileSink::createNew(envir(), *scs.session, this->videoname, fileSinkBufferSize0, ss->width, ss->height, 15, false, false, false, true);
		if (Out0 == NULL) {
			envir() << "Failed to create a \"QuickTimeFileSink\" for outputting to \""
...
void continueAfterDESCRIBE(RTSPClient* rtspClient, int resultCode, char* resultString) {
	do {
		...
		//Ϊ�Ự��������Դ���������ӣ���ÿ���ӻỰ�ϵ�������
		//����MediaSubsession::initiate()������Ȼ��Ϊÿ���ӻỰ����SETUP���ÿ���ӻỰӵ���Լ�������Դ
		scs.iter = new MediaSubsessionIterator(*scs.session);
...
void setupNextSubsession(RTSPClient* rtspClient) {
	...
	((ourRTSPClient *)rtspClient)->createVideoFile();  //�ļ�����
...
void continueAfterPLAY(RTSPClient* rtspClient, int resultCode, char* resultString) {
	...
	if (!success) {
		//����������ת�Ĵ���
		shutdownStream(rtspClient);
		//((ourRTSPClient *)rtspClient)->sendstartreply(-1);   //send fail message
	}
	else{
		//((ourRTSPClient *)rtspClient)->sendstartreply(1);   //send success message
	}
}
...
void shutdownStream(RTSPClient* rtspClient, int exitCode) {
	...
	//�����ط�Ҳ���ͷ�StreamClientState

	/*
	if (-- (((ourRTSPClient*)rtspClient)->rtspClientCount) == 0) {
		//��û������ʱ�˳����򣬻�����eventLoopWatchVariable = 1���滻������eventloop�Ƿ�������
